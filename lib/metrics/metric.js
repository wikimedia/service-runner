...
class Metric {
    ...
    increment(amount, labels) {
        ...
        if ([types.COUNTER, types.GAUGE].indexOf(this.type) !== -1) {
            ...
            this.logger('error/metrics', `increment() unsupported for metric type ${this.type}`);
        }
    }
    ...
    decrement(amount, labels) {
        ...
        if (this.type === types.GAUGE) {
            ...
            this.logger('error/metrics', `decrement() unsupported for metric type ${this.type}`);
        }
    }
    ...
    observe(value, labels) {
        ...
        if ([types.HISTOGRAM, types.SUMMARY].indexOf(this.type) !== -1) {
            ...
            this.logger('error/metrics', `observe() unsupported for metric type ${this.type}`);
        }
    }
    ...
    gauge(amount, labels) {
        ...
        if (this.type === types.GAUGE) {
            ...
            this.logger('error/metrics', `set() or unique() unsupported for metric type ${this.type}`);
        }
    }
    ...
    timing(value, labels) {
        ...
        if (this.type === types.TIMING) {
            ...
            this.logger('error/metrics', `timing() unsupported for metric type ${this.type}`);
        }
    }
    ...
    endTiming(startTime, labels) {
        ...
        if (this.type === types.TIMING) {
            ...
            this.logger('error/metrics', `endTiming() unsupported for metric type ${this.type}`);
        }
    }
}
