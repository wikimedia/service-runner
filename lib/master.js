...
class Master extends BaseService {
	constructor( options ) {
		...
		this._shutdownMasterHandler = () => {
			this.stop()
				.then( () => {
					this._logger.log( 'info/service-runner/master', { message: 'Exiting master' } );
	...
	stop() {
		...
		this._logger.log( 'info/service-runner/master', { message: 'master shutting down, killing workers' } );
	...
	_start() {
		...
		this._logger.log( 'info/service-runner',
			{ message: `master(${ process.pid }) initializing ${ this.config.num_workers } workers` } );
		...
		return ratelimiterSetup
			...
			.tap( () => {
				this._logger.log( 'warn/service-runner', { message: 'startup finished' } );
				...
				switch ( msg.type ) {
					...
					default:
						this._logger.log( 'error/service-runner/master',
							{ message: `unknown message type received from worker ${ msg.type }` } );
				}
			};
