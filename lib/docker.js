...
function createDockerFile() {
    ...
    customSourcePkgs.forEach(function(customSourcePkgSpec) {
        contents += 'RUN echo "deb ' + customSourcePkgSpec.repo_url +
                ' ' + customSourcePkgSpec.release +
                ' ' + customSourcePkgSpec.pool + '" > /etc/apt/sources.list;' +
                ' apt-get update; apt-get install -y --force-yes -t ' + customSourcePkgSpec.release +
                ' ' + customSourcePkgSpec.packages.join(' ') +
                ' && rm -rf /var/lib/apt/lists/*\n'
    });
